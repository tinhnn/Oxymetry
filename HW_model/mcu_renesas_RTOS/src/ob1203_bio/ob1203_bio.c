...
extern void ob1203_bio_delay_ms(uint32_t const delay_ms);
...
bool ob1203_bio_measurement_start(ob1203_bio_t *const p_bio)
{
    ...
    {
        ob1203_bio_i2c_callback_flag_clear();
        err = RM_OB1203_DeviceInterruptCfgSet(p_bio->p_instance->p_ctrl,
                                              p_bio->interrupt_cfg);
        if (FSP_SUCCESS != err)
        {
            return false;
...
bool ob1203_bio_measurement_stop(ob1203_bio_t *const p_bio)
{
    ...
    {
        ob1203_bio_i2c_callback_flag_clear();
        err = RM_OB1203_MeasurementStop(p_bio->p_instance->p_ctrl);
        if (FSP_SUCCESS != err)
        {
            return false;
        }
...
bool ob1203_bio_measurement_period_wait(ob1203_bio_t *const p_bio)
{
    ...
    do
    {
        ...
        {
            ob1203_bio_i2c_callback_flag_clear();
            err = RM_OB1203_DeviceStatusGet(p_bio->p_instance->p_ctrl,
                                            &device_status);
            if (FSP_SUCCESS != err)
            {
                return false;
...
bool ob1203_bio_ppg_raw_data_read(ob1203_bio_t *const p_bio, rm_ob1203_raw_data_t *const p_raw_data)
{
    ...
    if (RM_OB1203_PPG_INTERRUPT_TYPE_FIFO_AFULL == p_bio->interrupt_cfg.ppg_type)
    {
        ...
        {
            ob1203_bio_i2c_callback_flag_clear();
            err = RM_OB1203_FifoInfoGet(p_bio->p_instance->p_ctrl,
                                        &fifo_info);
            if (FSP_SUCCESS != err)
            {
                return false;
        ...
        if (OB1203_BIO_MAX_NUMBER_OF_SAMPLES_20 < fifo_info.unread_samples)
        {
            p_bio->number_of_samples = OB1203_BIO_MAX_NUMBER_OF_SAMPLES_20;
        }
        else
        {
            p_bio->number_of_samples = fifo_info.unread_samples;
        }
    }
    else
    {
    ...
    if (0 != p_bio->number_of_samples)
    {
        ...
        {
            ob1203_bio_i2c_callback_flag_clear();
            err = RM_OB1203_PpgRead(p_bio->p_instance->p_ctrl,
                                    p_raw_data,
                                    p_bio->number_of_samples);
            if (FSP_SUCCESS != err)
            {
                return false;
            }
                                   ...
                                   bool *const p_valid)
{
    ...
    *p_valid = false;
    ...
    if (0 != p_bio->number_of_samples)
    {
        ...
        err = RM_OB1203_PpgDataCalculate(p_bio->p_instance->p_ctrl,
                                         p_raw_data,
                                         p_ppg_data);
        if (FSP_SUCCESS != err)
        {
...
bool ob1203_bio_gain_currents_reconfigure(ob1203_bio_t *const p_bio, ob1203_bio_gain_currents_t *const p_gain_currents)
{
    ...
    {
        ob1203_bio_i2c_callback_flag_clear();
        err = RM_OB1203_GainSet(p_bio->p_instance->p_ctrl,
                                p_gain_currents->gain);
        if (FSP_SUCCESS != err)
        {
            return false;
        }
    ...
    {
        ob1203_bio_i2c_callback_flag_clear();
        err = RM_OB1203_LedCurrentSet(p_bio->p_instance->p_ctrl,
                                      p_gain_currents->currents);
        if (FSP_SUCCESS != err)
        {
            return false;
        }
